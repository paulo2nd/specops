...
- Q (checklist review): Is the terminal gate a deterministic verdict check or a human `gate`? → A: **Deterministic** — a `specops review`/verdict check that fails closed when the verdict is not `APPROVED`. It is **not** a human approve/reject gate (which could approve past a still-failing review); the human decides out-of-band after the halt (FR-019 refined).
- Q (analyze review): How does the workflow coexist with the injected Principle IV directives that also create the ledger and transition phases on the same lifecycle commands? → A: The **directives remain the sole owner** of forward-seam ledger creation and phase transitions; the workflow does **not** duplicate them, owning only the corrective REVIEW→IMPLEMENT round and additive records, with all workflow-issued `specops status` calls **idempotent-tolerant** (FR-008 refined + new edge case).
...
- **Coexistence with injected Principle IV directives**: In a client repo the injected lifecycle-hook directives (Constitution Principle IV) already create the ledger and advance phases at the standard seams, and they fire when the workflow runs the lifecycle commands. The workflow relies on them for those forward-seam transitions and does not re-issue them; it owns only the corrective REVIEW→IMPLEMENT round (which no directive performs) and additive records. Every workflow-issued `specops status` call is idempotent-tolerant, so if a directive already advanced the phase the workflow step is a no-op-and-continue, never a double transition.
...
- **FR-005**: The definition MUST interleave SpecOps deterministic gates (`review`, `consistency`) as steps at the lifecycle points pinned in the workflow contract (`contracts/specops-workflow.md`) — reconcile as a precondition before each state-changing step, review at the implementation seam and as the terminal gate — driven by their CLI outcome.
...
- **FR-008**: All deterministic state transitions (phase advances, task and review-cycle records) MUST be performed by SpecOps CLI directives — either the injected Constitution Principle IV lifecycle-hook directives that fire on the lifecycle commands, or explicit `specops status` steps in the workflow — and Spec Kit's engine MUST NOT write SpecOps ledger state. The workflow MUST NOT duplicate a transition already performed by an injected directive, and every workflow-issued `specops status` call MUST be idempotent-tolerant (a no-op-and-continue when the ledger is already in the target state).
