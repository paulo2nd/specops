...
- Q: Is the `policy` edge realizable against the current Feature 008 map schema? → A: **No — `policy` is defined and enforced but unpopulated.** Feature 008 models gates as a per-context identifier list, not cross-context policy edges, so scope is expanded by `ownership`/`dependency` edges only until a future schema field expresses a policy edge; gate/risk metadata is surfaced without adding any context to scope. (Reconciles the closed edge set with the real schema — planning finding C1.)
- Q: Does `context impact` take a `--phase` selector? → A: **No.** Impact is a property of the dependency graph over changed paths and is phase-independent; phase-scoped read sets are obtained separately via `context resolve --phase`/`context plan-check`. `--phase` applies to `plan-check` and `resolve`, not `impact` (planning finding U1).
...
3. **Given** a repository with no context map, **When** tasks and reviews are recorded, **Then** each record carries an explicit no-map provenance marker (`{map: none}`, not an omitted field) and no other ledger behavior changes.
...
- **FR-007**: SpecOps MUST make every context included in an expanded read or review scope attributable, in the reason trace, to exactly one member of the closed edge-type set **`ownership`** (a path match), **`dependency`** (a declared reverse dependency edge), or **`policy`** (a declared gate/policy edge). No context may appear in expanded scope without such an edge, and no edge type outside this closed set may pull a context into scope. The `policy` edge type is **defined and enforced but currently unpopulated**: the Feature 008 map models gates as a per-context identifier list, not as cross-context policy edges, so no `policy` edge can pull a *different* context into scope until a future map-schema field expresses one. Until then, expansion is realized by `ownership` and `dependency` edges only, and a context's own gate/risk metadata is surfaced without itself adding any context to scope.
...
- **SC-002**: 100% of contexts appearing in an expanded read or review scope are attributable to exactly one edge from the closed set {`ownership`, `dependency`, `policy`}; there are zero unexplained contexts (and zero contexts introduced by any out-of-set edge type) in expanded scope across the test fixtures. Because `policy` is currently unpopulated (FR-007), fixtures assert that only `ownership` and `dependency` attributions are produced and that no `policy` attribution appears until the schema supports it.
