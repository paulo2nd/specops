...
- Q: An imported finding is stale when the current effective diff no longer matches what it was reviewed against; staleness is computed against what? → A: **Per-finding, against that finding's own location/target** — a finding is stale only when the specific path/hunk it points at has changed since it was reviewed; an unrelated change elsewhere in the feature's effective diff does not stale it.
...
- **FR-004**: Each imported finding MUST record the **commit / effective-diff digest** it was reviewed against, captured at import from the input document (or the current effective diff when the producer supplies none), reusing the Feature 009/010 digest representation. The recorded digest MUST be scoped to the finding's **own location/target** (the path/hunk it points at), so staleness (FR-010) is evaluated per finding rather than against one feature-wide digest.
...
- **FR-010**: SpecOps MUST detect and report **staleness per finding**: when the current effective diff at an imported finding's **own location/target** no longer matches that finding's recorded reviewed-diff digest (FR-004), the finding MUST be reported as `stale`, surfacing the recorded vs. current digest. Staleness MUST be evaluated per finding against its location, so a change elsewhere in the feature's effective diff MUST NOT stale a finding whose target is unchanged. A stale finding MUST NEVER be silently trusted as describing the current diff; a stale promoted-blocking finding still participates in the gate but its staleness is reported (reusing the Feature 009/010 digest-drift pattern; SpecOps adds no new drift gate).
...
- **Reviewed-Diff Digest**: The commit / effective-diff digest an imported finding was reviewed against (Feature 009/010 representation); the basis for staleness comparison against the current effective diff.
...
- The effective-diff / commit-range **digest** representation is the one established by Features 009/010; staleness reuses that digest-drift pattern rather than introducing a new drift gate.
